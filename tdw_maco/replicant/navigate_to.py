...
from tdw.add_ons.replicant import TurnTo
...
class NavigateTo(MultiAction):
    ...
    _ACCEPTABLE_DISTANCE: float = 0.05
    ...
    def get_initialization_commands(self, resp: List[bytes], static: ReplicantStatic, dynamic: ReplicantDynamic,
        ...
        self.teleport_position = copy(self._target_position)
        # print("target_pos: ", static.replicant_id, self._target_position)
        # print("begin_pos: ", static.replicant_id, dynamic.transform.position)
        self._forward = dynamic.transform.forward.copy()
    ...
    def get_ongoing_commands(self, resp: List[bytes], static: ReplicantStatic, dynamic: ReplicantDynamic) -> List[dict]:
        ...
        elif self._navigation_state == _NavigationState.turn_back:
            return self._turn_back(resp=resp, static=static, dynamic=dynamic)
        elif self._navigation_state == _NavigationState.teleport:
            self._navigation_state = _NavigationState.end
            return [{"$type": "teleport_object", 
                     "position": {"x": self.teleport_position[0], "y": self.teleport_position[1], "z": self.teleport_position[2]}, 
                     "id": static.replicant_id}]
        elif self._navigation_state == _NavigationState.end:
    ...
    def _move_to(self, resp: List[bytes], static: ReplicantStatic, dynamic: ReplicantDynamic) -> List[dict]:
        ...
        if self._sub_action is None or self._sub_action.status == ActionStatus.success:
            ...
            if self._path_index == self._path.shape[0] - 1:
                self._navigation_state = _NavigationState.turn_back
                return []
            # Start the next MoveTo sub-action.
                ...
                arrived_at = 0.25 if self._path_index == self._path.shape[0] - 1 and isinstance(self.__target, int) and self.__target in self._state.target_object_ids else 0
                ...
                self._sub_action = MoveTo(target=self._path[self._path_index],
                                          ...
                                          collision_avoidance_distance=0.1,
                                          collision_avoidance_half_extents={"x": 0.1},
                                          collision_avoidance_y=0,
                                          previous=None)
    ...
    def _turn_back(self, resp: List[bytes], static: ReplicantStatic, dynamic: ReplicantDynamic) -> List[dict]:
        """
        Turn back to the starting position.

        :param dynamic: The dynamic Replicant data.
        """
        if not isinstance(self._sub_action, TurnTo) and self._sub_action.status == ActionStatus.success:
            self._sub_action = TurnTo(target=dynamic.transform.position + self._forward)
            ...
            if self._sub_action.status != ActionStatus.ongoing:
                self._navigation_state = _NavigationState.teleport
