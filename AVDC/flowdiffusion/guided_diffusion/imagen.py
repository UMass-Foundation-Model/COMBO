...
class PerceiverAttention(nn.Module):
    ...
    def forward(self, latents, mask = None):
        ...
        k, v = self.to_kv(latents).chunk(2, dim = -1)
...
class PerceiverResampler(nn.Module):
        ...
        *,
        ...
        if num_latents_mean_pooled > 0:
            self.to_latents_from_mean_pooled_seq = nn.Sequential(
                LayerNorm(dim),
                nn.Linear(dim, dim_latent * num_latents_mean_pooled),
                Rearrange('b (n d) -> b n d', n = num_latents_mean_pooled)
            )

        self.cross_attn = nn.ModuleList([
            CrossAttention(dim = dim_latent, context_dim = dim, dim_head = dim_head, heads = heads),
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PerceiverAttention(dim = dim_latent, dim_head = dim_head, heads = heads),
                FeedForward(dim = dim_latent, mult = ff_mult)
            ]))
    ...
    def forward(self, x, mask = None):
        ...
        if exists(self.to_latents_from_mean_pooled_seq):
            meanpooled_seq = masked_mean(x, dim = 1, mask = mask)
            meanpooled_latents = self.to_latents_from_mean_pooled_seq(meanpooled_seq)
            latents = torch.cat((meanpooled_latents, latents), dim = -2)

        attn, ff = self.cross_attn
        latents = attn(latents, context = x_with_pos, mask = mask) + latents
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context, mask = None, ret_score = False):
        b, n, device = *x.shape[:2], x.device
